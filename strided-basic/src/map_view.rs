...
///
/// This marker is not bound to a destination. It does not authorize unchecked
/// replay with a different geometry; callers of the unsafe execution entries
/// must establish that association themselves.
#[derive(Clone, Copy, Debug)]
pub struct ValidatedDestinationLayout(());
...
fn validate_destination_layout(
    ...
    if crate::layout_check::is_injective_layout(dims, strides) {
...
///
/// ```
/// use strided_basic::execution::validate_destination_layout_without_alloc;
/// let marker = validate_destination_layout_without_alloc(&[2], &[1]).unwrap();
/// assert!(validate_destination_layout_without_alloc(&[2], &[0]).is_err());
/// let _ = marker;
/// ```
///
/// # Errors
/// Returns `NonInjectiveOutputLayout` for invalid or unsupported output geometry.
pub fn validate_destination_layout_without_alloc(
    dims: &[usize],
    strides: &[isize],
) -> Result<ValidatedDestinationLayout> {
    if crate::layout_check::is_injective_layout_without_alloc(dims, strides) {
        Ok(ValidatedDestinationLayout(()))
    } else {
        Err(StridedError::NonInjectiveOutputLayout)
    }
...
#[cfg(test)]
#[path = "map_view/tests/scalar_branch_tests.rs"]
mod scalar_branch_tests;

#[cfg(all(test, feature = "parallel"))]
#[path = "map_view/tests/tests.rs"]
mod tests;
