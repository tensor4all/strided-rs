...
fn assert_map_arity_rejects_noninjective_destination(len: usize) {
    ...
    let mut dest = strided_basic::StridedViewMut::new(&mut output, &[len], &[0], 0).unwrap();
...
fn test_zip_map3_into_rejects_noninjective_destination_before_serial_write() {
    let sources = (0..3)
        ...
        .collect::<Vec<_>>();
    let mut output = [7.0_f64];
    let mut dest = strided_basic::StridedViewMut::new(&mut output, &[4], &[0], 0).unwrap();
...
fn test_strided_view_broadcast_and_copy() {
    ...
    let row = strided_basic::StridedView::<f64>::new(&data, &[1, 3], &[3, 1], 0).unwrap();
