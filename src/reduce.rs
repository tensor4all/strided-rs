...
use crate::promote::{broadcast_shape, broadcast_shape as broadcast_shape_dyn, promote_strides_to_shape};
...
/// Map-reduce (with broadcast + reduction stride-0) where the mapped value is produced by a
/// captured broadcast expression.
///
/// This is a Julia-style building block: a broadcast expression (including nested captures and
/// scalars) is evaluated per element, then reduced along dimensions where `dest` has size 1.
pub fn mapreducedim_capture1_into<T, SD, SA, LD, LA, C, R>(
    dest: &mut Slice<T, SD, LD>,
    a: &Slice<T, SA, LA>,
    capture: &C,
    reduce_fn: R,
    ...
    R: Fn(T, T) -> T,
{
    ...
    mapreducedim_capture_impl(&dst_view, &[a_view], capture, reduce_fn, init_op)
}

pub fn mapreducedim_capture2_into<T, SD, SA, SB, LD, LA, LB, C, R>(
    dest: &mut Slice<T, SD, LD>,
    a: &Slice<T, SA, LA>,
    b: &Slice<T, SB, LB>,
    capture: &C,
...
pub fn mapreducedim_capture3_into<T, SD, SA, SB, SC, LD, LA, LB, LC, C, R>(
    dest: &mut Slice<T, SD, LD>,
    a: &Slice<T, SA, LA>,
    b: &Slice<T, SB, LB>,
    c: &Slice<T, SC, LC>,
...
pub fn mapreducedim_capture4_into<T, SD, SA, SB, SC, SDd, LD, LA, LB, LC, LDd, C, R>(
    dest: &mut Slice<T, SD, LD>,
    a: &Slice<T, SA, LA>,
    b: &Slice<T, SB, LB>,
    c: &Slice<T, SC, LC>,
    ...
    R: Fn(T, T) -> T,
{
    ...
    let b_view = StridedView::from_slice(b)?;
    let c_view = StridedView::from_slice(c)?;
    let d_view = StridedView::from_slice(d)?;
    mapreducedim_capture_impl(
        &dst_view,
        &[a_view, b_view, c_view, d_view],
        capture,
        reduce_fn,
        init_op,
    )
...
fn mapreducedim_capture_impl<T, C, R>(
    dst_view: &StridedViewMut<T>,
    src_views: &[StridedView<T>],
    capture: &C,
    reduce_fn: R,
    init_op: Option<fn(&T) -> T>,
) -> Result<()>
...
{
    // Compute broadcast-compatible dimensions (dest may have size-1 reduction dims)
    let mut dims_list: Vec<&[usize]> = Vec::with_capacity(1 + src_views.len());
    dims_list.push(&dst_view.dims);
    for v in src_views {
        dims_list.push(&v.dims);
    }
    let dims = broadcast_shape_dyn(&dims_list)?;
    ...
    // Promote strides: stride-0 for broadcast or reduction.
    let mut promoted_strides: Vec<Vec<isize>> = Vec::with_capacity(1 + src_views.len());
    promoted_strides.push(promote_strides_to_shape(&dims, &dst_view.dims, &dst_view.strides)?);
    for v in src_views {
        promoted_strides.push(promote_strides_to_shape(&dims, &v.dims, &v.strides)?);
    }

    let strides_list: Vec<&[isize]> = promoted_strides.iter().map(|v| v.as_slice()).collect();
    let (fused_dims, plan) =
        build_plan_fused(&dims, &strides_list, Some(0), std::mem::size_of::<T>());

    let num_src = src_views.len();
    for_each_inner_block(&fused_dims, &plan, &strides_list, |offsets, len, strides| {
        let mut dst_ptr = unsafe { dst_view.ptr.offset(offsets[0]) };
        ...
        match num_src {
            1 => {
                let mut p0 = unsafe { src_views[0].ptr.offset(offsets[1]) };
                let s0 = strides[1];
            ...
            2 => {
                let mut p0 = unsafe { src_views[0].ptr.offset(offsets[1]) };
                let mut p1 = unsafe { src_views[1].ptr.offset(offsets[2]) };
                let s0 = strides[1];
                let s1 = strides[2];
            ...
            3 => {
                let mut p0 = unsafe { src_views[0].ptr.offset(offsets[1]) };
                let mut p1 = unsafe { src_views[1].ptr.offset(offsets[2]) };
                let mut p2 = unsafe { src_views[2].ptr.offset(offsets[3]) };
                let s0 = strides[1];
                let s1 = strides[2];
                let s2 = strides[3];
            ...
            4 => {
                ...
                let mut p1 = unsafe { src_views[1].ptr.offset(offsets[2]) };
                let mut p2 = unsafe { src_views[2].ptr.offset(offsets[3]) };
                let mut p3 = unsafe { src_views[3].ptr.offset(offsets[4]) };
                let s0 = strides[1];
                let s1 = strides[2];
                let s2 = strides[3];
                let s3 = strides[4];
            ...
            _ => {
                ...
                for i in 0..num_src {
                    src_ptrs.push(unsafe { src_views[i].ptr.offset(offsets[i + 1]) });
...
/// View-based Julia-style `mapreducedim!` for two sources with distinct element ops.
pub fn mapreducedim_capture2_views_into<'a, T, const N: usize, Op1, Op2, C, R>(
    dest: &mut StridedArrayViewMut<'_, T, N, Identity>,
...
where
    ...
    R: Fn(T, T) -> T,
{
    let dims = broadcast_shape(&[&dest.size()[..], &a.size()[..], &b.size()[..]])?;
    ...
    let dst_promoted_strides =
        promote_strides_to_shape(&dims, &dest.size()[..], &dest.strides()[..])?;
    let a_promoted_strides =
        promote_strides_to_shape(&dims, &a.size()[..], &a.strides()[..])?;
    let b_promoted_strides =
        promote_strides_to_shape(&dims, &b.size()[..], &b.strides()[..])?;

    let strides_list = [
        &dst_promoted_strides[..],
        &a_promoted_strides[..],
        &b_promoted_strides[..],
    ];
...
/// View-based Julia-style `mapreducedim!` for three sources with distinct element ops.
pub fn mapreducedim_capture3_views_into<'a, T, const N: usize, Op1, Op2, Op3, C, R>(
    dest: &mut StridedArrayViewMut<'_, T, N, Identity>,
    a: &StridedArrayView<'a, T, N, Op1>,
    b: &StridedArrayView<'a, T, N, Op2>,
...
where
    ...
    R: Fn(T, T) -> T,
{
    let dims = broadcast_shape(&[&dest.size()[..], &a.size()[..], &b.size()[..], &c.size()[..]])?;
    ...
    let b_promoted_strides =
        promote_strides_to_shape(&dims, &b.size()[..], &b.strides()[..])?;
    let c_promoted_strides =
        promote_strides_to_shape(&dims, &c.size()[..], &c.strides()[..])?;
...
/// View-based Julia-style `mapreducedim!` for four sources with distinct element ops.
pub fn mapreducedim_capture4_views_into<'a, T, const N: usize, Op1, Op2, Op3, Op4, C, R>(
    dest: &mut StridedArrayViewMut<'_, T, N, Identity>,
    a: &StridedArrayView<'a, T, N, Op1>,
    b: &StridedArrayView<'a, T, N, Op2>,
    c: &StridedArrayView<'a, T, N, Op3>,
    d: &StridedArrayView<'a, T, N, Op4>,
    capture: &C,
    reduce_fn: R,
    init_op: Option<fn(&T) -> T>,
) -> Result<()>
    ...
    R: Fn(T, T) -> T,
{
    ...
    let c_promoted_strides =
        promote_strides_to_shape(&dims, &c.size()[..], &c.strides()[..])?;
    let d_promoted_strides =
        promote_strides_to_shape(&dims, &d.size()[..], &d.strides()[..])?;
...
mod tests {
    ...
    #[test]
    fn test_mapreducedim_capture2_reduce_rows() {
        // Reduce rows: [3, 4] -> [1, 4]
        // mapped = a + b
        let a_data: Vec<f64> = (1..=12).map(|x| x as f64).collect();
        let a = Tensor::from_fn([3, 4], |idx| a_data[idx[0] * 4 + idx[1]]);
        let b = Tensor::from_fn([3, 4], |_| 1.0f64);
        let mut dest = Tensor::from_fn([1, 4], |_| 0.0f64);

        let capture = CaptureArgs::new(|x: f64, y: f64| x + y, (Arg, Arg));
        mapreducedim_capture2_into(&mut dest, &a, &b, &capture, |a, b| a + b, None).unwrap();

        // column sums(a) = [15,18,21,24]; column sums(b) = [3,3,3,3]
        assert_eq!(dest[[0, 0]], 18.0);
        assert_eq!(dest[[0, 1]], 21.0);
        assert_eq!(dest[[0, 2]], 24.0);
        assert_eq!(dest[[0, 3]], 27.0);
    }

    #[test]
    fn test_mapreducedim_capture1_reduce_rows() {
        let a: Tensor<f64, _> = Tensor::from_fn([3, 4], |idx| (idx[0] * 10 + idx[1] + 1) as f64);
        let mut dest: Tensor<f64, _> = Tensor::from_fn([1, 4], |_| 0.0);

        let capture = CaptureArgs::new(|x: f64| x, (Arg,));
        mapreducedim_capture1_into(&mut dest, &a, &capture, |a, b| a + b, None).unwrap();

        // Column-wise sums of:
        // [ 1  2  3  4
        //  11 12 13 14
        //  21 22 23 24 ]
        assert_eq!(dest[[0, 0]], 33.0);
        assert_eq!(dest[[0, 1]], 36.0);
        assert_eq!(dest[[0, 2]], 39.0);
        assert_eq!(dest[[0, 3]], 42.0);
    }
    ...
    fn test_mapreducedim_capture2_broadcast_rhs() {
        ...
        let a_data: Vec<f64> = (1..=12).map(|x| x as f64).collect();
        let a = Tensor::from_fn([3, 4], |idx| a_data[idx[0] * 4 + idx[1]]);
        let b_row = Tensor::from_fn([1, 4], |idx| (idx[1] as f64 + 1.0) * 10.0); // [10,20,30,40]
        let mut dest = Tensor::from_fn([1, 4], |_| 0.0f64);

        let capture = CaptureArgs::new(|x: f64, y: f64| x + y, (Arg, Arg));
        mapreducedim_capture2_into(&mut dest, &a, &b_row, &capture, |a, b| a + b, None).unwrap();

        // sum(a) cols = [15,18,21,24]; b contributes 3*[10,20,30,40]
        assert_eq!(dest[[0, 0]], 45.0);
        assert_eq!(dest[[0, 1]], 78.0);
        assert_eq!(dest[[0, 2]], 111.0);
        assert_eq!(dest[[0, 3]], 144.0);
    }
    ...
    #[test]
    fn test_mapreducedim_capture3_views_into_mixed_ops() {
        let a_data = vec![
            Complex::new(1.0, 2.0),
            Complex::new(3.0, -1.0),
            Complex::new(5.0, 4.0),
        ...
        let capture = CaptureArgs::new(|x: Complex<f64>, y: Complex<f64>, z: Complex<f64>| x + y + z, (Arg, Arg, Arg));
        mapreducedim_capture3_views_into(&mut out, &a, &b, &c, &capture, |a, b| a + b, None)
            .unwrap();

        // Sum of conj(a[i]) + b[i] + c[i]
        // conj(a) = [1-2i, 3+1i, 5-4i, 7+2i]
        // b = [1,2,3,4], c = [1,1,1,1]
        // sum = (1+1+1) + (3+2+1) + (5+3+1) + (7+4+1) = 30
        // imag = -2 + 1 - 4 + 2 = -3
        assert_eq!(out.get([0]), Complex::new(30.0, -3.0));
    }

    #[test]
    fn test_mapreducedim_capture4_views_into_mixed_ops() {
        let a_data = vec![
            Complex::new(1.0, 2.0),
            Complex::new(3.0, -1.0),
            Complex::new(5.0, 4.0),
        ...
        mapreducedim_capture4_views_into(
            &mut out,
            &a,
            &b,
            &c,
        ...
        // sum from previous test (30 - 3i) plus d (2 each * 4 = 8)
        assert_eq!(out.get([0]), Complex::new(38.0, -3.0));
    }
