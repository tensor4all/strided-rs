...
use strided_basic::{
    broadcast_mul_into, copy_into, copy_transpose_scale_into, map_into, mul_into, reduce,
    reduce_axis, with_execution_policy, zip_map2_into, zip_map3_into, ElementOp, ExecutionPolicy,
    Identity, StridedArray, StridedView, StridedViewMut,
};
use strided_fused::{fused_elementwise_into, FusedInst, FusedOp, FusedPlan, FusedScalar};
...
fn parallel_source_violations(relative: &std::path::Path, source: &str) -> Vec<String> {
    ...
    let threading_module = std::path::Path::new("strided-basic/src/threading.rs");
...
fn scanner_rejects_parallel_bypass_mutations() {
    ...
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap();
    let threading = std::fs::read_to_string(root.join("strided-basic/src/threading.rs")).unwrap();
    let extra_central_call = format!("{threading}\nfn bypass() {{ rayon::join(|| (), || ()); }}\n");
    assert!(!parallel_source_violations(
        std::path::Path::new("strided-basic/src/threading.rs"),
        &extra_central_call
    )
    .is_empty());
}
...
fn production_parallelism_is_routed_through_the_execution_policy_layer() {
    ...
    for owner in ["strided-basic", "strided-kernel", "strided-fused"] {
        visit(&source_root.join(owner).join("src"), &mut files);
    }
