...
| Method | Time (mean) | Throughput | Speedup vs Naive |
|--------|--------------|------------|------------------|
| naive | 5.21 ms | 201 Melem/s | 1.00× |
| strided_fused (`zip_map4_into`) | 2.47 ms | 425 Melem/s | **2.11×** (faster) |

**分析**: 4Dで出力が連続メモリの場合に、ブロッキングを回避してポインタ増分の4重ループで処理するパスを導入したことで、`zip_map4_into` が **約2.1倍高速**になりました。ブロック計画や多配列ストライドのオーバーヘッドが削減されています。
