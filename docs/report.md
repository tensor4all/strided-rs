...
**分析**: `strided` は `copy_transpose_scale_into_fast` を使用しており、naive より**約1.26倍高速**です。4x4マイクロカーネルによる最適化が効果的です。
...
| Method | Time (mean) | Throughput | Speedup vs Naive |
|--------|--------------|------------|------------------|
| naive | 0.991 ms | 1.06 Gelem/s | 1.00× |
| strided | 1.06 ms | 0.99 Gelem/s | **0.93×** (slower) |

**分析**: 4Dで出力が連続メモリの場合に、ブロッキングを回避してポインタ増分の4重ループで処理するパスを追加したことで、strided は **naive に近い性能**まで改善しました。まだわずかに遅いので、入力側のストライドを使ったタイル化や内側次元の連続性を活かす最適化が余地として残ります。
...
| Method | Time (mean) | Throughput | Speedup vs Naive |
|--------|--------------|------------|------------------|
| naive | 5.05 ms | 207 Melem/s | 1.00× |
| strided_fused (`zip_map4_into`) | 2.28 ms | 461 Melem/s | **2.22×** (faster) |

**分析**: 4Dで出力が連続メモリの場合に、ブロッキングを回避してポインタ増分の4重ループで処理するパスを導入したことで、`zip_map4_into` が **約2.2倍高速**になりました。ブロック計画や多配列ストライドのオーバーヘッドが削減されています。
...
6. **`zip_map4_into`**: 4Dの多順列和でも contig 出力なら高速（約2.1倍）

### 改善の余地

1. **4D配列の操作**: permutedims は依然遅く、非連続出力ではブロッキング最適化が必要
