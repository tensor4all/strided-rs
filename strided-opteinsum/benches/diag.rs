//! Diagonal extraction benchmark via opteinsum (ein"ijj->ij").
...
fn main() {
    println!("strided-opteinsum bench: diag (ijj->ij)");
    println!("Shape (100, 100, 100) -> (100, 100). Column-major.");
    println!();

    let n = 100usize;
    let shape_3 = [n, n, n];
    let code = parse_einsum("ijj->ij").unwrap();
