//! Index-sum benchmark via opteinsum (ein"ijk->ik").
...
fn main() {
    let n = 100usize;
    let shape_3 = [n, n, n];
    let code = parse_einsum("ijk->ik").unwrap();
    println!(
        "strided-opteinsum bench: indexsum (ein \"ijk->ik\"), large ({}^3)",
        n
    );
    ...
    bench_n("indexsum_Complex64_large", 1, 3, || {
        let result = code
            .evaluate(vec![EinsumOperand::from_view(&ac_view)], None)
            .unwrap();
        match result {
